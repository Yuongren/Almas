...
const AUDIO_CATEGORY_VALUES = AUDIO_CATEGORIES.map(
  (category) => category.value
);
function categoryLabel(value) {
  return AUDIO_CATEGORIES.find((category) => category.value === value)?.label ?? value;
}
async function fetchTracks() {
  ...
  console.log(
    "Supabase URL:",
    "https://btmduvshnmqgymxbkzfb.supabase.co"
  );
...
async function rateTrack(trackId, rating, userId) {
  ...
  if (existingError) {
    throw new Error(
      existingError.message || "Unable to check existing rating"
    );
  }
  if (existing) {
    ...
    if (error2) {
      throw new Error(
        error2.message || "Unable to update rating"
      );
    }
    return;
  }
  const { error } = await supabase.from("track_ratings").insert({
    track_id: trackId,
    user_id: userId,
    rating
  });
