...
import { createAudioRecord, listAudioRecords, saveAudioRecord, } from '../lib/db.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const uploadDir = path.resolve(__dirname, '../../uploads');
export const audioRoutes = async (fastify) => {
    /*
     * GET AUDIO
     */
     ...
     * UPLOAD AUDIO
     */
    fastify.post('/admin/audio/upload', async (request, reply) => {
        try {
            console.log('========================================');
             ...
             */
            if (!fileBuffer) {
                console.error('ERROR: No audio file was received');
            ...
            const isPaid = fields.isPaid === 'true' ||
                fields.isPaid === 'True';
            const skizaCode = fields.skizaCode?.trim() || null;
            /*
             ...
             */
            console.log('Saving audio metadata to Supabase...');
            ...
            if (insertError || !trackData) {
                console.error('Supabase database insert failed:', insertError);
            ...
            console.log('Audio metadata saved successfully:', trackData);
            /*
             * Local backup
             */
            try {
                ...
                console.log('Local audio backup created:', localPath);
                /*
                 * Save in in-memory library
                 */
                const record = saveAudioRecord(createAudioRecord({
                    title,
                    description,
                    category,
                    file_name: safeName,
                ...
                console.log('Audio upload completed successfully without local backup');
                console.log('========================================');
                return reply.code(201).send({
                    success: true,
                    item: trackData,
                    backup: null,
                    warning: 'Audio uploaded successfully, but local backup failed.',
                });
            }
        }
        catch (error) {
